/**
 * TwoGroupResults — displays results of a two-group comparison.
 * Shows group summaries, normality test results, t-test and
 * Mann-Whitney U statistics, and a plain-English interpretation.
 */
...
export default function TwoGroupResults({ results, onBack }: Props) {
  ...
  return (
    <div className="mt-8">
      ...
      {/* Plain-English interpretation — most prominent element */}
          ...
          return (
            <div key={group} className="p-4 rounded-xl border border-gray-800 bg-gray-900">
              ...
              <div className="space-y-1 text-sm">
                ...
                />
                {g.normality_p !== null && (
                  <Row label="Shapiro-Wilk p" value={g.normality_p} />
                )}
              </div>
            </div>
          )
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            <TestRow
                ...
                stat={results.t_test.statistic}
                p={results.t_test.p_value}
                significant={results.t_test.significant}
                recommended={results.recommended_test === "t-test"}
                />
                <TestRow
                name="Mann-Whitney U"
                stat={results.mann_whitney.statistic}
                p={results.mann_whitney.p_value}
                significant={results.mann_whitney.significant}
                recommended={results.recommended_test === "Mann-Whitney U"}
                />
          </tbody>
        </table>
      </div>
...
/** Simple label-value row for group summary cards. */
function Row({
  label,
  value,
  highlight,
...
/** A single row in the test results table. */
function TestRow({
  name,
  stat,
  p,
