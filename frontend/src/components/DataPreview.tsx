    ...
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
        {data.columns.map(col => {
          ...
          return (
            <span key={col.name} style={{ background: bgVar, color: txtVar, fontSize: 10, padding: "2px 9px", borderRadius: 6, border: `1px solid ${txtVar}22`, fontFamily: "var(--font-mono)", display: "inline-flex", alignItems: "center", gap: 5 }}>
              {col.name}
              {col.missing > 0 && <span style={{ color: "#f59e0b", fontSize: 9 }}>· {col.missing} {t("dataPreview.missing")}</span>}
            </span>
          )
        })}
      </div>
